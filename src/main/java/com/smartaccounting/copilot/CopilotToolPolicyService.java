...
public class CopilotToolPolicyService {
    ...
    public CopilotToolPolicyService(
        @Value("${smartaccounting.copilot.agent.policy.action-requires-approval:true}") boolean actionRequiresApproval,
        @Value("${smartaccounting.copilot.agent.policy.default-dry-run:false}") boolean defaultDryRun,
        @Value("${smartaccounting.copilot.agent.policy.action-approval-ttl-seconds:1800}") long actionApprovalTtlSeconds
    ) {
