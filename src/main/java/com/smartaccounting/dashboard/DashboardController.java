...
public class DashboardController {
    ...
    public ResponseEntity<List<KpiDto>> kpis(@PathVariable String role) {
        ...
        if (com.smartaccounting.tenant.TenantContext.tenantId() != null) {
            hit = dashboardCacheService.getKpis(com.smartaccounting.tenant.TenantContext.tenantId(), role.toLowerCase(), "default").isPresent();
        }
    ...
    public SseEmitter alertsStream(@PathVariable String role) throws IOException {
        ...
        emitter.send(SseEmitter.event()
            .name("alert")
            .data(Map.of("role", role, "message", "Connected to SmartAccounting alert stream", "ts", Instant.now().toString())));
        return emitter;
    }
}
