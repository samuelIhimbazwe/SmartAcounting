...
export function UserTenantManagementPage() {
  ...
  const roleMutation = useMutation({
    mutationFn: ({ userId, role }: { userId: string; role: Role }) => updateTenantUserRole(tenantId, userId, role),
    onSuccess: async () => {
  ...
  const inviteMutation = useMutation({
    mutationFn: () => inviteTenantUser(tenantId, { email: inviteEmail, role: inviteRole }),
    onSuccess: async () => {
  ...
  return (
    <section className="mx-auto w-full max-w-5xl space-y-4">
      <header>
        <h2 className="m-0 font-[var(--font-display)] text-2xl font-semibold text-neutral-900">{t('management.title')}</h2>
        <p className="m-0 mt-1 text-sm text-neutral-600">{t('management.subtitle')}</p>
      </header>
      ...
      <article className="rounded-2xl border border-[var(--border-subtle)] bg-white p-4 shadow-[var(--shadow-card)]">
        ...
        {tenantsQuery.isError && <p className="m-0 mt-3 text-xs text-amber-700">{t('management.tenantsUnavailable')}</p>}
      </article>
